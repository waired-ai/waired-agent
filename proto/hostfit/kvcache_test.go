...
// lowest precision first, narrowed by the build's list, by the host on
// ollama and by the GPU on vLLM, with the unquantised type always left.
func TestKVCacheChoices(t *testing.T) {
	...
	cases := []struct {
		...
		{"ollama, CPU only is f16 whatever the list", catalog.RuntimeOllama, qwen, cpu, nil, []string{"f16"}},
...
func TestResolveKVCacheType(t *testing.T) {
	...
	cases := []struct {
		...
		{"CPU only ignores q8_0", catalog.RuntimeOllama, qwen, cpu, nil, "q8_0", "f16"},
