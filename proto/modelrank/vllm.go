...
// host by default: KVFactorFP8 (0.5) when VLLMUsesFP8KV, else
...
// (KVFactorF16 for `--kv-cache-dtype auto`, KVFactorFP8 for fp8 e4m3 on
// Ada+). Callers pass it explicitly (like tp and gpuMemUtil): serving
// must reflect the operator opt-out, selection passes VLLMKVFactor.
//
...
// result is 1024-aligned via hostfit.MaxContextTokens; note the weights
// are pre-padded ×1.15 here (see vllmWeightOverhead) whereas that
// helper's ollama callers pass them raw against an overhead-reduced
// budget.
func VLLMMaxModelLen(
	...
	return hostfit.MaxContextTokens(weightGB*vllmWeightOverhead, kvBytesPerTokFP16, kvFactor, budgetGB)
}
