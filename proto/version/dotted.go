...
// lead to — enough for comparing an engine's reported version against a
// catalog floor, or a build against a release tag, without pulling in a
// full semver library.
//
// It lives in proto/ because the catalog's MinEngineVersion floors
// (catalog.Variant.MinEngineVersion) are read on BOTH sides of the wire:
// the agent applies the floor when it picks a variant to serve, and the
// control plane needs the same answer to avoid offering a model the
// device's agent would then refuse (waired-ai/waired#1225). A second
// implementation of the comparison in the other repository is exactly
// how the fit rules came to disagree before proto/hostfit existed
// (waired-ai/waired#942), and the shapes here are subtle enough that a
// re-derivation would differ: the dotted-core-only comparison in
// AtLeast, the fail-on-unparseable rule, the "ollama version is " and
// Debian-epoch tolerance, and dpkg's prerelease ordering rather than
// SemVer's.
//
// The agent's internal/version package predates this one and still
// carries the update-path callers plus the dpkg cross-check; folding it
// onto this one is tracked separately.
//
// # Version shapes
//
...
// the model unavailable with a message about a version they visibly
// have, which is a worse answer than admitting an engine that is, by its
// own version number, the thing the floor asked for.
//
// The catalog already relies on this, which is what makes it a contract
// rather than an accident: the agent's decision
// 20260816/2024-qwen3-8-takes-the-27b-band.md picks 0.32.13 as the ollama
// pin and records WHY 0.32.14-rc0 was not taken — because AtLeast is
// prerelease-blind, so authoring a prerelease as a floor would silently
// mean its release.
//
// So this package deliberately carries two orderings, and they are not
// in competition. Compare answers "which of these two builds is newer",
// where a prerelease must sort below its release or every rc-to-rc
// update reports "already up to date" (waired-agent#781). AtLeast
// answers "does this engine clear a floor", where the prerelease is
// noise on a number authored against the release. Use Compare for the
// first question and this for the second; neither is the other's
// fallback.
//
// (This doc said the opposite until waired-agent#970: that the behaviour
// was "a record of today's behaviour, not a contract" and that #804 was
// open. #804 was answered the day before, and this copy had no importer
// to notice. It has one now.)
//
