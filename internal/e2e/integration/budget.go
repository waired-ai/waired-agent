...
// (the CI fail-open guard). Since the auto crossing was retired
// (docs/decisions/20260903/0333-no-automatic-crossing-to-or-from-anthropic.md)
// the blackhole is no longer a trap that a fail-open falls into and then
// hides: a turn addressed to Waired is never sent upstream at all, and a turn
// addressed to the real API is never dragged back. What it does now is give
// an upstream-claiming leg its terminus on a runner with no credentials —
// the proxy's own 502 saying it could not reach the host.
//
// expect is what the leg claims about its turn, and it changes which
// responses are terminal in BOTH directions: a leg that claims local must
// never produce an upstream-unreachable failure, and a leg that claims
// upstream must not be served here (waired-agent#1141).
func classifyDrive(status int, hdr http.Header, body []byte, blackholed bool, expect Outcome) (driveVerdict, string) {
	...
	// The proxy could not reach the real API. On an upstream-claiming leg
	// that IS the leg's answer on a runner with no credentials and a
	// blackholed host; on a local-claiming leg it is the escape the sentinel
	// exists to catch.
	if status == http.StatusBadGateway && strings.Contains(string(body), "waired_upstream_unreachable") {
		if expect == outcomeUpstream {
			return driveOK, ""
		}
		why := "the request left this machine for api.anthropic.com, and this leg claims to be " +
			"served here — nothing but a turn naming a real Anthropic model may leave " +
			"(docs/decisions/20260903/0333-no-automatic-crossing-to-or-from-anthropic.md)"
		if blackholed {
			why += "; this lane blackholes api.anthropic.com, which is what turned the escape into a 502"
		}
	...
	case http.StatusBadRequest: // 400
		// Since the auto crossing was retired this is also how a turn Waired
		// cannot serve ends (gateway.writeFailClosed). Terminal either way:
		// neither a rejected request nor an absent node clears by retrying.
		return driveTerminal, "HTTP 400 — the request is rejected on its merits, or nothing on Waired could serve it; neither clears by retrying"
