...
//
// The staging and swap that used to live here moved up into
// OllamaInstaller.Install (staged_install.go), where all three OSes now
// share it: the reasons Windows had for it — a running engine holds a
// mandatory lock on ollama.exe and on every DLL it has loaded, antivirus
// holds a handle on the directory, and a half-extracted tree still has an
// ollama.exe in it that the next run reads as a complete install (#190) —
// turned out to have a macOS twin, where the daemon exec'd a half-written
// Mach-O (waired-agent#1309). This function now only unpacks.
