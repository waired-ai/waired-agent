...
func TestRouteForModelID(t *testing.T) {
	for _, tc := range []struct {
		...
		{DirectiveModelAny, RouteWaired, true, "any of your own nodes is still Waired"},
		{Tier1M(DirectiveModelAny), RouteWaired, true, "the retired 1M spelling, same side"},
		...
		{LegacyModelCloud, "", false,
			"the retired cloud row names neither side: relaying it needed a " +
				"model rewrite, and waired-agent#1186 removed it"},
		{"claude-waired-cloud", "", false, "and its bare spelling is what arrives"},
		...
		{"waired/subagent", "", false, "the retired subagent label names no side either"},
