...
// TestCountOpenAIPromptTokensApprox_MatchesTheAnthropicCount keeps the
// two guards from disagreeing. The overlay sees bodies AnthropicToOpenAI
// produced on some other node, so a different convention here would fail
// a prompt the requester passed — or pass one it failed.
func TestCountOpenAIPromptTokensApprox_MatchesTheAnthropicCount(t *testing.T) {
	req := AnthropicRequest{
		MaxTokens: 64,
		Messages: []AnthropicMessage{
			{Role: "user", Content: json.RawMessage(`"` + strings.Repeat("word ", 100) + `"`)},
	...
	got, want := CountOpenAIPromptTokensApprox(encoded), CountTokensApprox(req)
	if got != want {
		t.Errorf("openai count %d vs anthropic count %d; the two guards would disagree "+
			"about the same conversation", got, want)
	}
