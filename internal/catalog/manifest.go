...
//
// A Manifest is the immutable description of a model (id, aliases,
// licence, context length, capabilities, runtime preference, variants,
// security settings) — see waired_inference_spec.md §5 and
// waired_product_spec.md §10.4. Phase A ships a single bundled
// manifest (`bundled/qwen3-8b.json`) embedded into the binary; CP
// catalog fetch and OS-global override paths are deliberately out of
// Phase A scope.
//
// The local State (state.json on disk, see local.go) tracks which
// model variants are present, downloading, or failed, plus which
// engine endpoints are currently active.
package catalog
...
// BundledManifests decodes every JSON file under internal/catalog/bundled
...
func (m *Manifest) Validate() error {
	...
	for i, v := range m.Variants {
		...
		if v.MinEngineVersion != "" && !version.Valid(v.MinEngineVersion) {
...
// contains is a small helper used by tests and validate.
