...
- 公開共有のピアはこの面でも擬名で出る。レポートは
  ...
  いずれも出ないことを主張する。
- **アカウント行**は当初この回の対象外にしたが、オーナーの指摘
  （「メールアドレスの行もグレーアウトしたままなのが気になります」)で
  同じ回に含めた。行き先は状態ダイアログではなく**コンソールのアカウント
  ページ**（`<control>/admin/account`）。daemon-down のメニューでは
  `AccountURL` を空にする — そこに出るメールは**最後に見えたもの**であって
  今回の poll が確認した identity ではないので、開ける先が無い。
- **範囲外**: メニュー先頭の見出し行（`● Connected`）だけがグレーのまま
  残った。同じ議論はこの行にも当てはまるので、必要になったら同じ形で直せる。
- **検討して採らなかった案**: 「inactive / off の行（○）をグレーにすれば
  正常かどうかが一目で分かる」というオーナーの提案。**グレーの行はクリックを
  受け取れない**（Windows は `MFS_DISABLED` で OS がイベントを出さず、macOS は
  `enabled=FALSE`、Linux は shell が `enabled` を尊重して何も送らない）ので、
  「グレーにする」と「クリックで状態が見える」は同じ行では両立しない。
  オーナーの判断で**全行クリック可能を優先**し、正常/異常の区別は記号
  （● ○ ◐ ⚠）と先頭行の ⚠ に任せた。
