...
| kv offloading | 起動して配信する／プール > 窓／**argv に両フラグ+`native` が載っている** |
| 横断 | **チャンクが大きいほどプールが小さい** |

エンジンが実効チャンクを自分で印字するので、#955 が求めた
「エンジンが報告するフラグが導出値と一致」は**直接**主張できる。
横断のプール比較はその裏取りで、**届かなかったフラグは 2 本とも同じプールを出す** —
「起動した」だけを見るテストがフラグの黙った脱落を通してしまう穴を、ここで塞ぐ。

**オフロードはプールで主張しない(初回 run で訂正)。** 当初は「オフロードを入れると
GPU プールが減る」を横断の主張に入れていた。20B 級モデルの実測 −15.7% からの
一般化だったが、**成り立たなかった**: L4 + Qwen0.5B では 3.7 GiB のバッファ有無で
プールが **1,485,888 トークンのまま同一**(run 33242543508)。バッファはホスト RAM で、
GPU 側の代償があってもこの構成のブロック粒度より小さい。
**1 ホスト 1 モデルの性質にレーンを依存させるのは、このテストが避けようとしている当の誤り**
なので撤去し、代わりに **argv に載っていること + エンジンが ready に達したこと**で主張する。
vLLM の argparse は未知のオプションを拒否するので、配信まで到達した spawn は
両フラグを**受理している** — 上流でのリネームや削除はこれで捕まる。
...
## 初回 run で得た事実 (33242543508)

- **`nvcc` は GPU ランナーの PATH に無く、`/usr/local/cuda/bin` に在る。** それでも
  vLLM 0.28.0 は 6 ターゲット全部で起動した。`processEnv` は
  `VLLMAdapter` の spawn 内(`internal/runtime/vllm.go:419`)で呼ばれるので、
  **`NewVLLMAdapter` を呼ぶ者は誰でも CUDA bin 注入を受ける** — 自前で
  `VLLMConfig` を組み立てる e2e も含む。「e2e は本番と同じ env 経路に乗っているか」
  という懸念(#1131 由来)は**乗っている**で決着。足した `nvcc provenance` の 1 行が
  これを答えられるようにした。
- **実効チャンクの echo は実在する**: 導出 4096 / override 512 とも一致。
- **`usage.prompt_tokens_details` は出る**: `{"cached_tokens":0,"created_cache_tokens":32,…}`。
- **チャンク→プールの向きは実測で成立**: 4096 → 1,485,888 / 512 → 1,509,120。
- **レーン増分は 3.4 分**(206s)。予算 90 分に対し全体 28.7 分なので、
  `full` に相乗りさせたままで問題ない(専用 dispatch に分ける必要は無かった)。
