...
> このsweepの仕組みについてはやはり廃止、[Strix Halo のホスト]型で性能劣化に気が付けないのもやむなしとしたいです。（つまり新規の深さを追加はしない）
...
- **1 スロットのホストではエンジンを占有する。** Strix Halo のホスト（Windows の Strix Halo APU、
...
- 深さ 0 では対話床を超えるが、深さで這うホストは、もう気づかれない。持ち主に軽い
  モデルも提案されない。実測例は Strix Halo のホストの 1 件: 浅い decode 81.5 tok/s（床 60）に
