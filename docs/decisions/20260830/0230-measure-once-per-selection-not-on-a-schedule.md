...
docs/decisions/20260913/2245-speed-is-one-request-at-32768-tokens.md（オーナー裁定 2026-09-13、決定 7）が「測る条件は選択が変わったときだけ」の鍵と寿命を**部分的に狭める（覆さない）**: 鍵は（variant SHA, エンジン版, GPU）になり、計測値はデーモンの外に永続化されて再起動・モデル切替・切替の取り消しをまたいで再利用される。利用者の再実行だけがその鍵を上書きする。「周期では測らない」「失敗も判定（ただしメモリ内のみで、永続化しない）」「同期の 1 回とキャッシュヒット」「deps は毎回読み直す」はそのまま有効。
