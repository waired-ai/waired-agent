...
### 6. 失敗率は試行数によらないが、「モデルの欠陥」という解釈は誤り

qwen2.5-coder 0.5b/3b/7b/14b は n=12 で全 tool 呼び出し・全試行が失敗する
（100%）。**この数字自体は試行数を増やしても動かない。**

ただしこの note の初版は、ここから「退役の根拠は無傷」と結論していた。
**それは誤り**なので訂正する。失敗の中身を見ると、これらのモデルは

```json
{"name": "Read", "arguments": {"file_path": "/etc/hostname"}}
```

を返している — **正しい tool を正しい引数で選んでいて、構造化されずに
地の文に出ているだけ**。gateway が `tool_calls` の空を検知して本文を
解析すれば回収できる（waired-ai/waired-agent#409）。

したがって agentgrade が測っているのは **(モデル, ハーネス) の組** であり、
モデル単体の性質ではない。失敗は 2 種類に分かれる:

- **クラス A** — エンジンは 200 を返し、呼び出しは本文にある。gateway
  側で回収可能。qwen2.5-coder 全サイズ、qwen3-coder:30b、granite4:350m。
- **クラス B** — エンジンが 500 を返し、解析すべき本文が存在しない。
  qwen3.5:9b/4b/35b-a3b。#409 の範囲外。

退役の判断は #409 の着地後の再測定でやり直す必要がある。
