...
`UD-Q3_K_XL` は「3bit の variant」ではなく、**1bit から 8bit までを層ごとに混ぜて
3.90 bits/param に収めた variant**で、FFN テンソルの約 1 割が既に 2bit に落ちている。
`UD-Q2_K_XL` も同様に 1bit を 20 本含む。

したがって:

- **`quantization_tier` の整数は、サイズの段としては正しいが、精度の測度としては
  正しくない。** 混在する variant に 1 個の整数を貼っている。
- **同じ「3bit」というラベルで、別の発行元の variant と横に並べることはできない。**
...
ただし 4bit どうしの比較は一方的ではない。**公式の variant は一様に `Q4_K` で 4bit 未満が
無い**代わりに attention も `Q4_K` 止まりで、**`UD-Q4_K_M` は attention を `Q5_K`〜`Q8_0`
で厚く守る**代わりに FFN に `Q3_K` を 7 本含む。**どちらを既定にするかは、どこを守る
variant を選ぶかという判断**であって、片方が一様に優れているわけではない。

### 同じタグでも、同じ大きさのモデルの間で中身が違う

同じ `arch=qwen35`・851 テンソル・同じパラメータ数の 27B を 3 本、同じ
`UD-Q3_K_XL` というタグで読み比べた:

| `UD-Q3_K_XL` | bits/param | FFN の最低 | 2bit のテンソル |
|---|---|---|---|
| **カタログが出荷しているもの** | **3.90** | `IQ2_XXS` / `IQ2_S` / `IQ2_XS` | **23 本** |
| 同世代の別モデル A | 4.29 | `IQ3_S` | **0 本** |
| 同世代の別モデル B | 4.28 | `IQ3_S` | **0 本** |

**差はモデルの大きさではなく、ビルドごとの切り方にある。** 出荷しているものが
3 本の中で最も深く切られており、2bit を含むのはそれだけ。

したがって、この variant で測った劣化を **「3bit 量子化の性質」とも
「そのサイズのモデルの性質」とも読んではいけない**。読めるのは
**そのファイルの性質**だけ。

## Correction (20260914)

初出時、2bit テンソルが混ざる理由を「埋め込みと attention は params に比例して
縮まないので、大きいモデルほど同じタグに収めるために深く切る必要がある」と
説明した。**これは誤り。** 同じ大きさ・同じアーキテクチャの兄弟 2 本が 2bit を
1 本も含まないので、サイズの法則では説明できない。上の節に差し替えた。
ノートの主張の本体（タグは精度ではなくサイズの目標であり、`quantization_tier`
は精度の測度ではない）は、この訂正でむしろ強まる — 同じタグが、同じサイズの
モデルの間でも別物を指していたため。

### MoE の variant は router と attention を守っているが、dense の variant は守らない
...
route 由来」と言う。**出荷されている MoE の variant は MoQE 側の構成をそのまま採って
いる**ので、後者の警告は当たらない。dense の 2bit の variant にはその保護が無い。
