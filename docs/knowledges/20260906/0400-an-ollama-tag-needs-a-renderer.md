...
> **訂正 (2026-09-06 21:00)。この節の 3 つの主張は誤り。**
> `docs/knowledges/20260906/2100-the-qsa-indexer-adds-a-third-kv-cache.md`
> が上書きする。要点だけ:
>
> 1. 2 つ目は **QSA indexer で確定**。MTP ではない。llama.cpp b10760 の
>    `src/llama-model.h` にレイヤーの value 射影が無く、`src/models/qwen4exp.cpp`
>    が `cpy_k` / `get_k` しか発行していない。
> 2. **9216 は導出できる。** K と V の幅が違うだけで、`12 × (128 + 256) × 2 B`
>    に一致する。`2 × L × h × d × 2` に当てはめようとしたのが誤りだった。
> 3. **38 % ではない。** 本物は indexer のキー 3072 B/token (12.5 %) だけで、
>    残る 6144 は上流が未使用の V を確保しているもの (ggml-org/llama.cpp#28330、
>    b10760 時点で open)。カタログの注記は 27648 で、33792 ではない。
...
> **訂正 (2026-09-06 18:30)。この節はもう現状ではない。**「upstream の 1 フィールド
> 待ち」は誤りで、待つ先は存在しなかった。下の補足が置き換える。レーンは
> waired-agent#1253 / #1259 で着地済み。
