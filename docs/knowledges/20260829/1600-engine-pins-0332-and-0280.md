...
- **`--kv-offloading-size 8 --kv-offloading-backend native`**: 同じあふれ方
  でも、先行 2 会話は再送で**両方とも完全に温かい** — 120,066 中 120,064
  cached で 0.18 s、120,064 中 120,063 cached で 0.14 s。
- 対価: **GPU プールを減らすという証拠は無い**。当初ここには「プールが
  285,883 に減る」と書いていたが、取り下げた。理由は 2 つあり、下の追記
  2 本を読むこと。
...
## 追記 2 (20260829 18:30) — オフロードは GPU プールを減らさない

追記 1 では「285,883 を代償として引くのはやめる」と書いた。その後、
**別レーン (waired-agent#955) が独立に測って、代償そのものが無い**ことが
出た。逐語で受け取った結果:

> L4 + Qwen0.5B で、3.7 GiB のバッファ有無で **1,485,888 トークンのまま
> 同一**。

別のカード・別のモデル・別の測り手で、**バッファを足してもプールが 1 トークン
も減らない**。バッファはホスト RAM にあるので、GPU 側の代償があるとしても
...
ドライバが解放を報告した時点と実際に使えるようになる時点は同じではない
らしい。**この 1 点は再現していないので、断定はしない。**

**動かない結論**: オフロードが**保持を買う**ことは変わらない。対照が
「あふれた後の再送が冷えるか温かいか」で、プールの絶対値に依存しないため。
waired-agent#955 も同じ理由で、主張を数値でなく**観測できるもの**
(argv に両フラグが載り、エンジンが ready に達して配信したこと) に張り替えた
と報告を受けている。

**手口として残す**: 1 台 1 構成で採った差分を「その設定の代償」と名付ける
前に、**別の構成で符号だけでも再現するか**を見ること。ここでは 2 段階
かかった — 1 段目 (追記 1) で「値が信用できない」まで、2 段目 (この追記)
で「効果そのものが無い」まで。
