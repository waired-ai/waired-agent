...
もう 1 つ、作業の途中で fleet にダウンロード禁止のオーナー指示 (2026-09-20) が入り、計測 1 件がそれで完了できなかった (§7)。
...
### 7. 測れなかった 1 件 — QSA indexer

`ollama_version.go` は「AT THE NEXT BUMP」と明示した指示を持っていて、この bump がその bump である: ggml-org/llama.cpp#28330 (コミット 311d4211b、b10889 で初出) は b10969 の祖先**である** — 実リポジトリに対する `git merge-base` で確認した。だから indexer の使われない V 半分は消えているはずで、カタログの `qwen3.8-flash-next` の 27648 B/token の注記は今なら実測と一致するはずである (b10760 ではエンジンは 33792 を持っていた)。

確認はできていない。その重みは約 78 GB で fleet のどのホストにも無く、ダウンロード禁止のオーナー指示が 2026-09-20 に、pull が約 50 GB まで進んだところで入った。pull は止めた。

これを後回しにしても何も払わない理由をはっきり書く: カタログは意図して、実測値ではなく**導出できる**数を刻んでいる (docs/knowledges/20260906/2100-the-qsa-indexer-adds-a-third-kv-cache.md §3)。だから未確認の修正は注記もコードも変えない。まだ分からないのは、エンジンが今 2 つの数字のどちらを持っているかだけ。`ollama_version.go` の指示は消さず、残ったその 1 手順に絞った。同記録の §4 は未確認のまま生きている。
