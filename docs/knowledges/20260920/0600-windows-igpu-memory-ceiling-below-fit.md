...
- **residency の warm-up は、同じ読み込みが続けて失敗するたびに間隔を
  ...
  start は回数によらず即座に温める。
- **変えていない: 予算。** この host class の予算は 98,304 MB (96 GiB)
  のままで、上の上限より約 12 GB 上にある。いったんは測った上限
  (80 GiB) まで下げる実装を書いたが、取り下げた。オーナーの判断
  (2026-09-20):
  ...
  機種ごとに定数を置くやり方は、機種が増えるたびにコードと実機の往復が
  要る。代わりに、足りなくなった時点で製品が止めて人に伝える方向で設計
  する (#1453)。予算は推定のままにし、外れたときの被害を仕組みで抑える。
