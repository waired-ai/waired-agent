...
- **どのヒープのどの確保が失敗しているか**、**予算の合計まで届かない
  理由**は分かっていない。§4 の読みは状況からの推定で、再現して確かめた
  原因ではない。
- 次の一手: 失敗する構成を `GGML_VK_MEMORY_LOGGER=1` で回して失敗する
  確保を名指しし、`GGML_VK_PREFER_HOST_MEMORY=1` で上限が動くかを見る。
  動くなら、製品は予算を下げる代わりに Windows の統合 GPU 機でこの
  環境変数を渡す選択肢がある。
- カタログの入れる条件がこの host class に使う予算は今も 98,304 MB
  (96 GiB。`internal/hardware/uma_common.go` の `strixHaloUMACapMB`、
  `internal/hardware/catalog_admission_test.go`) で、測った上限より約
  12 GB 上。どうするかはオーナーの判断待ちで、上の診断の結果で変わり
  得る。
