...
tmux があるホストなら、これが一番楽。`capture-pane` は端末が描いた**画面**を
返すので、下に書く再構成が要らない。
...
### tmux が無いホスト（このフリートの macOS 機）の罠

`expect` で代替できるが、**Claude Code の TUI は単語の間にカーソル移動を挟んで
描画する**。生ストリーム上では `auto mode on` が

```
auto\e[11Gmode\e[16Gon
```

になっている。したがって:

- **複数語の `expect` パターンも `grep` も空振りする。** 「REPL が立ち上がったか」を
  `auto mode on` で待つと必ずタイムアウトする。`agents` のような**単語 1 つ**で
  マッチさせること。ピッカーが開いたことの判定も同様（`Switch between` は不可。
  ただし `Switch` は起動バナーの「Switch anytime with /model」にも出るので、
  `recommended` のようにピッカーにしか出ない語を選ぶ）。
- **画面として読むにはカラム移動を再生する必要がある。** エスケープを単に削ると
  単語が連結して `Wairedpeer:linux-gpu(...)` になる。`\e[<n>G` を桁位置として
  適用し、行を組み直してから読む。
- 信頼プロンプトのあとに全画面のリリース告知が挟まることがある。固定 `sleep` では
  なく、REPL のマーカーを待ってから `/model` を送ること。

`script` も `screen` も同じ問題を持つ（端末エミュレーションをしないため）。
tmux が入れられるなら入れたほうが早い。

## 実装後の確認（2026-08-20）

1〜8 は設計前の測定。実装後、`0.0.3-edge.20260820120916` の macOS 実機で
**本物のライタ → 実メッシュ → 実キャッシュ → 実ピッカー**が 1 台で通ることを
確認した。ピッカーの行番号がそのまま並び順の証拠になる（組み込み 6 行の下に
5 つの固定エントリ、その下に peer 別行）。生データは waired#1226 のコメント。
