...
| notice / notices (daemon が公開する短い伝言 — `waired status` の `Notices:` ブロック、tray 上部の行、`waired doctor` の ⚠ 行) | お知らせ。製品出力の `Notices:` と各行の文(`⚠ Lighter model recommended — switch to <model-id>` / `⬆ Better model available — switch to <model-id>` とその下の一文)は逐語 | daemon が「いま事実であること」を tray・`waired doctor`・`waired status` の 3 面に同じ一覧で出す仕組み(`internal/notice`)。消す操作は無い — 産出側が繰り返し公開しなくなれば消える(lease)。**「通知」と書かない** — `internal/platform/notification` の OS のデスクトップ通知(一度出て消えるポップアップ、ja 既訳は「ポップアップ通知」)は別物で、同じ語にすると同じ機構に読める。doctor に出るのは警告だけ、Claude Code の面には出さない | オーナー裁定(20260905; waired-ai/waired-agent#1205; `docs/decisions/20260905/0000-a-notice-is-published-and-lapses.md`) |
