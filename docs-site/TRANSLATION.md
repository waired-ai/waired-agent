...
| Weights about {W} + KV cache about {K} + engine overhead about {O} (旧 Weights about {W} + KV cache about {K} / Model about {W} + session cache about {K}) | 重み約{W} + KVキャッシュ約{K} + エンジンのオーバーヘッド約{O} | ピッカー行の事実行(スペック表記・全行同一)。`device_weights_mb`、`kv_cache_mb`、`required_window_resident_mb` から前 2 つを引いた残り(compute バッファ、再帰状態、MTP の draft、fit の余裕)の内訳。オーバーヘッドを KV cache と呼ばない。「セッションキャッシュ」「コンテキストキャッシュ」「作業用メモリ」は使わない — 標準語は KV cache。CLI / tray の接尾辞は `· 2.5 GB in system RAM`(旧 `· 2.5 GB of KV cache in system RAM`。llama.cpp の fit がシステム RAM に置くのは KV ではなく層なので) | waired#1174 裁定(20260812)→ オーナー裁定(20260822; waired-ai/waired#1272; `docs/decisions/20260822/2029-user-copy-uses-standard-llm-terms.md`)→ オーナー裁定(20260914; waired-ai/waired-agent#1337; `docs/decisions/20260914/1200-vram-sizing-follows-the-fit.md` 決定 9) |
...
| Waired peer (`/model` の項目名) | **Waired peer**（逐語・訳さない） | 製品出力の引用なので上の `peer` 行の適用外。ラベル `Waired peer`・説明 `Another of your computers`（旧 `Waired peer (another device)`、その前は `Waired peer (another device, no local fallback)` — auto ルートの撤去で「fallback」の語ごと落ちた; waired-agent#1184）。名前付きの行はラベル `Waired peer: <name>`・説明にそのコンピュータのモデル名（旧 `Waired peer: <name> (<model>)`）。かつて説明欄は private cache 経由で `From gateway` に固定されていたためラベル 1 行に全部を収めていたが、`modelPicker` の行には説明欄があるので分けた（waired-agent#1185）。1M を宣言するコンピュータがあるときは `<ラベル> (1M context)` の双子行が出る — これも逐語。`waired/peer` の行も `Waired peer: <name>` の行も、200,704 トークン以上のコンテキストウィンドウを持つコンピュータだけを使い、200,704 トークンを示す(`Waired local` を含むすべての行と同じ条件。`(1M context)` の双子行は 1,048,576)。どのコンピュータが答えるかに依らない。`Waired peer: <name>` の行は 200,704 トークン以上を宣言するコンピュータにだけ出る(旧: `Waired peer: <name>` の行には条件が無く、そのコンピュータ自身のコンテキストウィンドウを示した)。例外: コンテキストウィンドウが 200,704 未満のカスタムモデルを動かすコンピュータには `Waired local` と `Waired peer: <name>` の行が出て、そのモデル自身のコンテキストウィンドウを示す(名指ししない行は 200,704 のまま; waired-ai/waired#1473 裁定 10、waired-ai/waired#1481) | オーナー裁定(20260820、セッション内; waired-agent#830) → waired-agent#1184 → オーナー裁定(20260906; waired-agent#1185) → オーナー裁定(20260916; waired-ai/waired-agent#1395; `docs/decisions/20260916/0410-the-window-floor-is-the-same-on-every-client.md`) → オーナー裁定(20260916; waired-ai/waired-agent#1396; `docs/decisions/20260916/2350-every-waired-row-is-a-200k-or-1m-session.md`) |
| Waired public share (`/model` の項目名) | **Waired public share**（逐語・訳さない） | 〃。ラベル `Waired public share`・説明 `Someone else's computer`（旧 `Waired public share (someone else's computer)`）。Public Share を有効にしたホストにだけ出る条件付きの項目なので、ja 側でも件数を書かない。自分が使える公開コンピュータが 1M を宣言しているときは `Waired public share (1M context)` の双子行が出る(旧「双子行は無い」は撤回: 公開コンピュータもコンテキストウィンドウを公開している)。この行も 200,704 トークン以上のコンテキストウィンドウを持つ公開コンピュータだけを使う(`Waired local` と `Waired peer: <name>` を含むすべての行と同じ条件) | オーナー裁定(20260820、セッション内; waired-agent#901) → オーナー裁定(20260906; waired-agent#1185) → オーナー裁定(20260916; waired-ai/waired-agent#1395; `docs/decisions/20260916/0410-the-window-floor-is-the-same-on-every-client.md`) → オーナー裁定(20260916; waired-ai/waired-agent#1396; `docs/decisions/20260916/2350-every-waired-row-is-a-200k-or-1m-session.md`) |
...
| Custom models / Import a model (Hugging Face からアカウントに取り込んだモデルと、その操作) | カスタムモデル / インポート | 製品の見出し `Custom models`（Waired アプリの［Models］、`waired models ls --detail`、`waired init` の一覧）は逐語。地の文の概念は「カスタムモデル」、操作は「インポート」。「独自モデル」「取り込み」としない | オーナー裁定(20260922; waired-ai/waired#1473 の裁定 1) |
...
| KV cache (旧 context cache / session cache / working memory) | KV キャッシュ | 推論中にトークンごとに保持するメモリ。glossary に定義。`models ls --detail` の凡例は `"in system RAM" is how much of a full coding session this computer's VRAM cannot hold.`(旧 `"KV cache in system RAM" is the part of a full coding session this computer's GPU cannot hold.`) | オーナー裁定(20260822; waired-ai/waired#1272; `docs/decisions/20260822/2029-user-copy-uses-standard-llm-terms.md`)→ オーナー裁定(20260914; waired-ai/waired-agent#1337) |
...
| default variant | 既定の variant | モデルごとにカタログが名指す variant(`default_variant`)。モデルだけを選んだときに使われる。ただしメモリに収まらないパソコンでは小さい variant が使われるので、「常に既定の variant が動く」とは書かない。量子化の名前(`Q4_K_M`、`UD-Q2_K_XL` など)は逐語で、「軽い」「高速」のような向きのある語を添えない | オーナー裁定(20260913; waired-ai/waired#1357; `docs/decisions/20260913/2355-catalog-variant-kv-and-residency-rulings.md`) |
| KV cache type | KV キャッシュの種類 | `f16` / `q8_0` / `q4_0`(Ollama)、`fp16` / `fp8`(vLLM)は逐語。glossary の KV キャッシュの項に既定を書いている | オーナー裁定(20260913; waired-ai/waired#1357; `docs/decisions/20260913/2355-catalog-variant-kv-and-residency-rulings.md`) |
...
| `waired/default` / `waired/local` / `waired/peer` / `waired/peer-<名前>` / `waired/public` (OpenCode / OpenClaw のモデル) | 逐語(識別子)。表示名 **Waired** / **Waired local** / **Waired peer** / **Waired peer: &lt;名前&gt;** / **Waired public share** と区分名 **Waired** も逐語 — 上の `/model` の項目名の行(210〜213)と同じ語で、同じ表を 3 つのクライアントが出すため。説明は「ターンが動く場所」を言う(「このパソコン」「自分の別のパソコン」「他の人のパソコン」)。**「モデル」と説明しない** — 指しているのはパソコンで、どのモデルで答えるかはそのパソコンが持っているもの | 旧 `waired/coding` / `waired/small` は #521 で退役済みなので書かない。`waired/default` の旧表示名 **Waired Default** も書かない(any-node 行として **Waired** に揃えた)。表示名・設定に書く ref として bare `waired` は OpenCode / OpenClaw では書かない — 両者はモデルを `<provider>/<model>` で指すので第 2 区画が要る。ただし**ワイヤに送る id は別**: プラグインは `waired/default` を `waired`、`waired/default[1m]` を `waired[1m]`(Claude Code が送るのと同じ id)として送り、この id が 200k / 1M の条件を運ぶ。チャットアプリが直接送る `waired/default` に条件は無い。`(1M context)` の双子行(`waired/<key>[1m]`)は 3 つのクライアントで同じに出る | 今日の挙動の記録(20260823) → waired-ai/waired-agent#1306(20260912; `internal/integration/opencode/templates/plugin_waired.js.tmpl`) → オーナー裁定(20260916; waired-ai/waired-agent#1395) |
...
| notice / notices (daemon が公開する短い伝言 — `waired status` の `Notices:` ブロック、tray 上部の行、`waired doctor` の ⚠ 行) | お知らせ。製品出力の `Notices:` と各行の文(`⚠ Faster model recommended — switch to <model-id>`(旧 `⚠ Lighter model recommended — switch to <model-id>`。線を超えたときに勧める移り先が、今より重いモデルになりうるので `Lighter` を `Faster` に改めた) / `⬆ Update available — install <version>` / `⚠ <engine> is not running the version Waired pins` / `⚠ <engine> is serving a configuration this computer could not hold` / `⬆ <engine> sized this model for this computer` とその下の一文)は逐語 | daemon が「いま事実であること」を tray・`waired doctor`・`waired status` の 3 面に同じ一覧で出す仕組み(`internal/notice`)。消す操作は無い — 産出側が繰り返し公開しなくなれば消える(lease)。**「通知」と書かない** — `internal/platform/notification` の OS のデスクトップ通知(一度出て消えるポップアップ、ja 既訳は「ポップアップ通知」)は別物で、同じ語にすると同じ機構に読める。doctor に出るのは警告だけ、Claude Code の面には出さない。**エンジンの調整（tuning）の行は、収まらなかったときだけ ⚠** — 意図した取引は ⬆ で、これは severity が `TuningDegraded` で決まるため。tray の更新バナー（旧 `⚠ Update available — install vX`）はこの一覧の行になったので、⚠ ではなく ⬆ | オーナー裁定(20260905; waired-ai/waired-agent#1205; 追記 20260906; waired-ai/waired-agent#1229; `docs/decisions/20260905/0000-a-notice-is-published-and-lapses.md`) → オーナー裁定(20260916; waired-ai/waired-agent#1400; `docs/decisions/20260916/0340-catalog-reference-host-rank-and-admission.md`) |
