...
| `waired status` | Show daemon + identity status. Add `--observability` for the live engine/mesh snapshot. On a service install the device state is root-owned, so run it with `sudo` (elevated on Windows) — without elevation it exits with a permission message instead of guessing. |
