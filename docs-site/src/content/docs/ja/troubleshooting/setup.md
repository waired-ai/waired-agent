...
| ページの表示 | 意味 | 対処 |
|---|---|---|
| 「…のセットアップコマンドが完了する前に終了しました。進捗は保存されています。」 | セットアップを実行していたターミナルが閉じられました。一部の手順には管理者権限が必要で、そのターミナルだけがそれを持っています。 | `sudo waired init`をもう一度実行します（Windowsでは管理者のターミナルで`waired init`）。続きから再開し、失われるものはありません。 |
...
| `4` | サインイン済みですが、誰も答えなかった質問でセットアップが止まりました。このページの問題ではなく、完了時のカードが示すフラグを付けてもう一度実行します。[キーボードなしで答える](/ja/getting-started/set-up-in-the-terminal/#answering-without-a-keyboard)を参照してください。 |
