...
- **`loading the model (16s)`**：重みをメモリに読み込んでいます。数字はその経過時間です。推論エンジンの起動後の最初のリクエストが何も表示されないまま待たされていたのは、この時間です。大きいモデルで読み出しの遅いディスクだと数分かかります。
- **`model not loaded`**：重みがメモリに無く、読み込みも走っていません。Wairedが数秒のうちに自分で読み込みを始めるので、この表示が続くことはありません。続く場合は推論エンジンがそのモデルの読み込みを拒んでいます。1分ほど待って**Engine**の行をもう一度見て、それでも変わらなければ[モデルを変更する](/ja/guides/choose-a-model/)を参照してください。
...
- モデルは答える前にメモリに読み込まれる必要があります。読み込み中かどうかは上の**Engine**の行に出ます。その間、ネットワーク上のほかのパソコンはこのパソコンに仕事を送りません。
