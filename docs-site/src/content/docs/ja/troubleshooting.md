...
直し（Windows は管理者プロンプトから）、Claude Code を再起動してください。自分の
エンジンを持たないコンピュータでは、この行は `none here` と表示し、代わりにほかの
コンピュータから借りている上限を示します。ここには会話を保持するものが無いので、
届く範囲でいちばん小さい上限が正直な数だからです。
