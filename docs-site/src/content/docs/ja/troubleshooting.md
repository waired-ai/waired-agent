...
サービスインストール(Linux/macOS で通常の `sudo waired init` を行ったケース)では、
デバイス状態は root だけが読めるシステムディレクトリ(Linux では
`/var/lib/waired`)に保存されます。一般ユーザーで `waired status` や
`waired auth status` を実行してもこれを読めないため、コマンドは対象ディレクトリと
対処(`sudo waired status`、Windows では管理者プロンプトで再実行)を示す
permission エラーで終了します。
