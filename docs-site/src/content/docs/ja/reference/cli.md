...
メモリに載らないモデルは、不足量を示したうえで**さらに強い確認**を求めます
（既定は No）— ダウンロード完了後の読み込みに失敗する見込みだからです。この
確認は `--yes` だけでは省略できません。本当に実行したいスクリプトは
...
Waired アプリの両方に即時反映され、再起動後も保持されます。終わったら `info` に
...
バックグラウンドサービスのログ（システムログから）と AI エンジンのログを集めます。
macOS では `/Library/Logs` にあるサービス自身のログファイル（およびアプリの
`~/Library/Logs`）も、ローテーション済みの古い分まで含めて回収します。最後の
ローテーションより前に始まった問題も報告に残ります。
