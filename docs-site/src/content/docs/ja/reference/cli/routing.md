...
一時停止は、このパソコンのすべてのルーティングを止めます。答えるのをやめ、Wairedに送ったターンは、自分のほかのパソコンが受けないかぎり失敗します。設定は再起動後も保持されます。バックグラウンドサービスが動いていないときは、選択が保存されて次の起動時に適用されます。

```
waired-agent not running — pause persisted; will apply on next start.
```

「止める」が意味する5つの違いは、[Wairedを一時停止する](/ja/guides/pause/)を参照してください。
