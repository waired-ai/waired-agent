...
通常のデスクトップのインストールでは状態はシステムのものなので、すべてを見るには`sudo`で、Windowsでは管理者のターミナルで実行します。管理者権限がないとそのフォルダを読めないので、代わりにバックグラウンドサービスに尋ね、サービスの答えをそのまま報告します。システム全体でサインインしている、サインインしていない、あるいは — サービスも答えないときは — ここからは判別できない、のいずれかです。
...
サインアウトすると、このパソコンは答えるのをやめます。ローカル推論は停止し、Claude Codeの`/model`の一覧からWairedの行が外れるので、実行先のないモデルが提示されることはありません。それでもセッションが要求した場合は、このパソコンがサインアウトしていることと`waired init`を示す答えが返ります。

サインアウトがあえて触らないものが2つあります。1つはClaude Code連携そのものです。これを変更するには管理者権限が必要で、サインアウトは管理者権限を求めないからです。Claude Codeはそのまま動き、Anthropicのモデルは今までどおりクラウドへ行きます。連携を外すには`waired claude disable`を実行してください。もう1つはデバイス一覧で、このパソコンはサインアウト済みとして残ります。サインインし直すのがコマンド1つで済むためです。戻る予定がなければ一覧から削除してください。次の`waired init`は新しいデバイスとして登録され、古い項目が名前を保持したままなので、新しい方が`<パソコン名>-1`になることがあります。
