...
/**
 * Size class per model_id, generated by `catalog-tool docs` from the same
 * manifests this loader reads. Regenerate with `make catalog-docs`.
 */
const MODEL_SIZES: Record<string, string> = JSON.parse(
	readFileSync(join(process.cwd(), 'src', 'data', 'model-sizes.json'), 'utf8'),
);
...
export interface CatalogRow {
	 ...
	 *
	 * Read from the generated model-sizes.json rather than computed here.
	 * The rule lives in Go (proto/hostfit.ModelSize) and this loader is
	 * the one place that reads the bundled JSON directly, so recomputing
	 * it would be a second implementation of a rule the product enforces
	 * elsewhere — the shape of waired#942. `catalog-tool docs --check`
	 * fails if the data file drifts from the manifests.
	 */
...
export function loadCatalog(): CatalogRow[] {
	...
	const rows: CatalogRow[] = files
		...
		return {
			...
			paramsLabel,
			modelSize: MODEL_SIZES[m.model_id] ?? null,
