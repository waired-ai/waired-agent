...
func bounceProvider(t *testing.T, r *blockingRunner) *agentInferenceProvider {
	...
	// The state dir this one writes into is pullGateProviderWithRunner's,
	// taken above and therefore removed after everything arm waits for.
	_, agentCtx, arm := providerLifetime(t)
	p.agentCtx = agentCtx
	// This fixture reaches endPull, so it takes the same lifetime as the
