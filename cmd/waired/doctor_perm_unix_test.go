...
func TestCollectDoctorFindings_UnreadableStateDirIsNotAMissingIdentity(t *testing.T) {
	...
	// Other rows legitimately fail on a locked state dir (the gateway
	// token is unreadable too), so the assert is that THIS row adds
	// nothing to the count — a skip never does.
