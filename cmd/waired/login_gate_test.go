...
func TestPresentLoginURL_PrintOnly(t *testing.T) {
	...
	presentLoginURL(nil, &out, "https://cp.example/login/abc", "XKCD-42", "https://cp.example", false, gatePrintOnly)
	if *calls != 0 {
...
func TestPresentLoginURL_PrintOnlyReadsNothing(t *testing.T) {
	...
	waitQueued(t, s, 1)
	presentLoginURL(s, &out, "https://cp.example/login/abc", "", "https://cp.example", false, gatePrintOnly)
...
func TestLoginGate_PrintOnlyAcksAStrayEnterOnPoll(t *testing.T) {
	...
	g := presentLoginURL(s, &out, "https://cp.example/login/abc", "", "https://cp.example", false, gatePrintOnly)
	waitQueued(t, s, 1)
	g.Poll(&out)
...
func TestPresentLoginURL_AutoOpen_URLPrintedBeforeOpen(t *testing.T) {
	...
	t.Cleanup(func() { openBrowserFn = orig })
	presentLoginURL(nil, &out, "https://cp.example/login/abc", "", "https://cp.example", false, gateAutoOpen)
...
func TestLoginGate_PromptDoesNotBlock(t *testing.T) {
	...
	s := newStdinReader(strings.NewReader("\n"))
	waitQueued(t, s, 1)
	g := presentLoginURL(s, &out, "https://cp.example/login/abc", "", "https://cp.example", false, gatePrompt)
	if *calls != 0 {
...
func TestLoginGate_NilAndOwnerlessAreInert(t *testing.T) {
	...
	for _, mode := range []browserGate{gatePrintOnly, gateAutoOpen, gatePrompt} {
		g := presentLoginURL(nil, &out, "https://cp.example/login/abc", "", "https://cp.example", false, mode)
		g.Poll(&out)
		g.Withdraw(&out)
	}
